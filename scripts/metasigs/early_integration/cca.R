...
get_clusters <- function (data, n_cluster, start = 1e-4, end = 2., epsilon = 1e-8){
  for (i in 1:10){
    log_info("Trying randomseed ", i, ".")
    results <- binary_search_leiden(data = data, n_cluster=n_cluster, start = start, end = end, epsilon = epsilon, random_seed = i)
    if (!is.null(results)){
      return(results)
    }
  }
...
integrated <-get_clusters(integrated, opt$n)
